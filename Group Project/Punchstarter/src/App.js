...
function App() {
  ...
  function AdminPage() {
    ...
    const handlereapProjects = () => {
      ...
      let dataValue = JSON.stringify(msg);
      let data = { 'body': dataValue }
      instance.post('/reapProject', data).then((response) => {
        if (response.data.statusCode === 200) {
          window.alert("Projects Reaped")
        }
        console.log("lol")
      })
      GetProjectsAdmin(setData);
    ...
    return (
      <div>
        ...
        <div style={{
          ...
          <div>
          <button style = {layout.reapProjectsButton} onClick={handlereapProjects}>Reap Projects</button>
      ...
      <div>
        ...
        <div style={layout.designerListProject}>
          {CurrentJSON.Projects.map((Project, index) => {
            ...
            console.log("Project", Project.Project_ID,launched, Project.IsLaunched)
            if (launched === 0) {
              return (
                <div key={index} style={{
            ...
            }else if (Project.IsLaunched === '-'){
              console.log("dlfjdflkd")
              return (
                <div key={index} style={{
                  backgroundColor: '#383A38', borderRadius: 10, padding: 20, borderColor: 'black',
                  borderWidth: 3,
      ...
      <div>
        ...
        <div style={layout.designerTransactions}>
          ...
          {currentProject.Transactions.length >= 1 ? currentProject.Transactions.map((Transaction, index) => {
            ...
            return (
              <div style={{ color: 'white' }}>
                <p>'{Transaction.Supporter_ID}' bought a pledge '{Transaction.Pledge_ID.substring(0, Transaction.Pledge_ID.length()-Transaction.Project_ID.length)}' worth ${Transaction.Amount}</p>
              </div>
            )
          })
            : <div>
      ...
      return (
        <div>
          <div>
            ...
            <p style={layout.supportedProjectsText}>Supported Projects and Claimed Pledges:</p>
            <button style={layout.supporterActivityBackButton} onClick={handleToActivityPage}>Back</button>
            <div>
            </div>
          </div>
          <div style={layout.supportedProjects}>
                ...
                return (
                  <div key={index} /* style={layout.pledgesClaimed} */>
                    <Link to={`/supporter/${Pledge.Project_ID}`}>
                      <div style={layout.claimedPledgesList}>Claimed Pledge: '{Pledge.Pledge_ID.substring(0, Pledge.Pledge_ID.length-Pledge.Project_ID.length)}' from {Pledge.Project_ID} worth ${Pledge.Amount}</div>
                    </Link>
                  </div>
                );
              })}
